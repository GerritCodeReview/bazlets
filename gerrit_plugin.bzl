...
def gerrit_plugin_with_ext_deps(
        plugin,
        ext_deps = [],
        ext_repo = None,
        deps = [],
        **kwargs):
    """Builds a Gerrit plugin with external deps declared as Maven coordinates.

    This is a convenience wrapper around `gerrit_plugin()` that allows external
    dependencies to be expressed as Maven coordinates instead of explicit
    generated Bazel labels.

    If `ext_repo` is not set, it defaults to `<plugin>_plugin_deps`.

    Args:
      plugin: Plugin name as passed to `gerrit_plugin()`. Used as the plugin
        target name and to derive the default external dependency repository.
      ext_deps: List of Maven coordinates for external dependencies, for example
        `["com.github.scribejava:scribejava-core"]`.
      ext_repo: Name of the external repository generated by
        `rules_jvm_external`. Defaults to `<plugin>_plugin_deps`.
      deps: List of additional Bazel dependencies for the plugin.
      **kwargs: Additional arguments passed to `gerrit_plugin()`.
...
def gerrit_plugin_tests_with_ext_deps(
        plugin,
        name = None,
        srcs = [],
        deps = [],
        ...
        **kwargs):
    """Runs junit tests for a Gerrit plugin with external deps as Maven coordinates.

    This is a convenience wrapper around `gerrit_plugin_tests()` that allows
    external test dependencies to be expressed as Maven coordinates instead of
    explicit generated Bazel labels.

    The wrapper automatically:
      - derives the test target name as `<plugin>_tests` if `name` is not set
      - adds the plugin target `:{plugin}__plugin` to test dependencies
      - derives `ext_repo` as `<plugin>_plugin_deps` if not set
      - derives `tags` as `[plugin]` if not set

    Optionally, this macro also creates runtime dependency safety tests by
    calling `gerrit_plugin_dependency_tests()` for the same plugin.

    Args:
      plugin: Plugin name as passed to `gerrit_plugin()`. Used to derive the
        default test target name, plugin target dependency, default external
        dependency repository, default tags, and dependency test targets.
      name: Optional name of the junit test target. Defaults to
        `<plugin>_tests`.
      srcs: List of Java source files for the plugin tests.
      deps: List of additional Bazel dependencies for the test target.
      ext_deps: List of Maven coordinates for external test dependencies, for
        example `["com.github.scribejava:scribejava-core"]`.
      ext_repo: Name of the external repository generated by
        `rules_jvm_external`. Defaults to `<plugin>_plugin_deps`.
      tags: Optional list of tags for the test target. The plugin tag is always
        added automatically if not already present.
      exports: List of targets to export for in-tree testing. Must be used
        together with `plugin` argument and follows the semantics of
        `gerrit_plugin_tests()`.
      create_dependency_tests: Whether to also create
        `gerrit_plugin_dependency_tests()` targets for `plugin`. Defaults to
        True.
      dependency_test_name: Name of the generated dependency test suite.
        Defaults to `"dependency_tests"`.
      ...
      dependency_test_overlap_against: Optional overlap manifest passed to
        `gerrit_plugin_dependency_tests()`.
      **kwargs: Additional arguments passed to `gerrit_plugin_tests()`.
    ...
    gerrit_plugin_tests(
        ...
        srcs = srcs,
        plugin = plugin,
        deps = [":%s__plugin" % plugin] + deps + _artifacts(ext_deps, ext_repo),
        exports = exports,
        tags = tags,
        **kwargs
    )
