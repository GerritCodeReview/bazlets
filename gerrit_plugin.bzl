...
def _expected_ext_deps_java_library_name(plugin):
...
def gerrit_plugin_ext_test_deps(
    ...
    """Defines external dependencies required for a Gerrit plugin test suite.

    Args:
      plugin: Name of the plugin under test (required).
      ext_deps: List of Maven coordinates (group:artifact) for external test dependencies (required).
      name: Name of the java_library rule. Usually, should not be set. Will be
        set automatically to the value expected for Eclipse classpath generation.
    ...
    """
    expected_name = _expected_ext_deps_java_library_name(plugin)
    if name == None:
        name = expected_name
    elif name != expected_name:
        print("WARNING: gerrit_plugin_ext_test_deps: `name` is not as expected `" + expected_name + "`. This may cause the eclipse classpath generation to fail.")
...
def gerrit_plugin_tests(
    ...
    Args:
      ...
      plugin: Name of the plugin under test. Only required if `name` is not set
        to generate default clean name.
      ext_deps: Reference to a java_library rule exporting the external test deps.
        Defaults to the rule with the name expected by Eclipse classpath generation
        ("<plugin name>__plugin_test_deps")
    ...
    expected_ext_deps_name = ""
    if plugin:
        if name == None:
            name = plugin + "_tests"
        expected_ext_deps_name = _expected_ext_deps_java_library_name(plugin)
    elif name == None:
    ...
    if not ext_deps and native.existing_rule(expected_ext_deps_name):
        ext_deps = ":" + expected_ext_deps_name
