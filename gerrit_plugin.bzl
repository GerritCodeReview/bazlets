...
def _label_for_compare(label):
    """Return a stable string form for comparing existing rule labels.

    `native.existing_rule().get("exports")` stringifies labels in a form that
    depends on whether the external repo is visible from the current macro
    evaluation context. The repo prefix can appear as `@<repo>//`, `@@<repo>//`,
    or `@@[unknown repo '<repo>' requested from @@]//`. Normalize these forms
    while preserving the apparent repo name so different repos do not compare
    equal just because they export the same package + target.
    """
    if label == None:
        return None
    ...
    if s.startswith(":"):
        ...
        return "//%s%s" % (package_name, s) if package_name else s

    unknown_repo_prefix = "@@[unknown repo '"
    if s.startswith(unknown_repo_prefix):
        repo_start = len(unknown_repo_prefix)
        repo_end = s.find("'", repo_start)
        target_start = s.find("//")
        if repo_end != -1 and target_start != -1:
            return "@%s%s" % (_repo_for_compare(s[repo_start:repo_end]), s[target_start:])

    if s.startswith("@@"):
        return _repo_label_for_compare(s[2:])

    if s.startswith("@"):
        return _repo_label_for_compare(s[1:])

    return s

def _repo_for_compare(repo):
    """Normalize canonical repo names to the apparent names used by callers."""
    rules_jvm_external_repo_prefix = "rules_jvm_external++maven+"
    if repo.startswith(rules_jvm_external_repo_prefix):
        return repo[len(rules_jvm_external_repo_prefix):]
    return repo

def _repo_label_for_compare(label_without_at):
    """Normalize an external label after the leading `@` or `@@` was removed."""
    target_start = label_without_at.find("//")
    if target_start == -1:
        return "@" + _repo_for_compare(label_without_at)
    return "@%s%s" % (
        _repo_for_compare(label_without_at[:target_start]),
        label_without_at[target_start:],
    )

def _labels_for_compare(labels):
    """Normalize and sort labels, ignoring absent optional attributes."""
    return sorted([_label_for_compare(label) for label in labels if label != None])

def _check_existing_rule_attrs(context, rule_name, attrs):
    """Fail if an existing generated rule differs from a repeated macro call."""
    existing = native.existing_rule(rule_name)
    if not existing:
        fail("%s: expected generated rule %r to already exist" % (context, rule_name))

    for attr, new_value in attrs.items():
        existing_value = existing.get(attr)
        if type(new_value) == "list":
            existing_value = _labels_for_compare(existing_value or [])
        elif type(new_value) != "bool":
            existing_value = _label_for_compare(existing_value)

        if existing_value != new_value:
            fail(
                ("%s: rule %r already exists with a different %s value. All repeated " +
                 "macro calls creating this rule must use the same value.\n" +
                 "  existing: %s\n" +
                 "  new:      %s") %
                (context, rule_name, attr, existing_value, new_value),
            )
...
def gerrit_plugin_tests(
    ...
    Args:
      ...
      ext_deps: List of Maven coordinates for external test dependencies.
        When set, dependency tests are generated automatically. Repeated calls
        for the same plugin may reuse generated rules only when `ext_deps` and
        dependency-test options resolve to the same labels.
      ext_repo: Name of the external repository generated by rules_jvm_external.
      ...
      skip_dependency_tests: Whether to skip generating dependency tests when
        `ext_deps` and `plugin` are set. Defaults to `False`. This is an
        explicit opt-out, not required for repeated calls with matching inputs.
    ...
    if ext_deps or exports:
        ...
        existing = native.existing_rule(test_deps_name)
        if existing:
            _check_existing_rule_attrs(
                "gerrit_plugin_tests(plugin = %r)" % plugin,
                test_deps_name,
                {"exports": _labels_for_compare(exports)},
            )
        else:
            java_library(
                name = test_deps_name,
...
def gerrit_plugin_dependency_tests(
    ...
    Repeated calls for the same plugin reuse generated test targets only when
    the target, allowlist, overlap manifest, and suite contents match.

    Args:
      plugin: Plugin name as passed to `gerrit_plugin()`. Used to derive the
              checked target `:{plugin}__plugin` and to name the generated
              test targets.
      name: Name of the generated test suite. Defaults to "dependency_tests".
            Multiple names may create multiple suites for the same plugin, but
            the underlying dependency test targets must still match.
    ...
    # The manifest attrs mirror tools/java_runtime_jars_manifest.bzl defaults
    # used by runtime_jars_allowlist_test().
    allowlist_exists = native.existing_rule(allowlist_test)
    allowlist_manifest_exists = native.existing_rule(allowlist_manifest)
    if allowlist_exists or allowlist_manifest_exists:
        context = "gerrit_plugin_dependency_tests(plugin = %r)" % plugin
        _check_existing_rule_attrs(
            context,
            allowlist_manifest,
            {
                "exclude_self": True,
                "normalize": True,
                "target": _label_for_compare(plugin_target),
            },
        )
        _check_existing_rule_attrs(
            context,
            allowlist_test,
            {"data": _labels_for_compare([allowlist, ":%s.txt" % allowlist_manifest])},
        )
    else:
    ...
    overlap_manifest = overlap_test + "_manifest"

    # The manifest attrs mirror tools/java_runtime_jars_manifest.bzl defaults
    # used by runtime_jars_overlap_test().
    overlap_exists = native.existing_rule(overlap_test)
    overlap_manifest_exists = native.existing_rule(overlap_manifest)
    if overlap_exists or overlap_manifest_exists:
        context = "gerrit_plugin_dependency_tests(plugin = %r)" % plugin
        _check_existing_rule_attrs(
            context,
            overlap_manifest,
        ...
        _check_existing_rule_attrs(
            context,
            overlap_test,
            {"data": _labels_for_compare([":%s.txt" % overlap_manifest, overlap_against])},
        )
    else:
    ...
    if native.existing_rule(name):
        _check_existing_rule_attrs(
            "gerrit_plugin_dependency_tests(plugin = %r)" % plugin,
            name,
            {"tests": _labels_for_compare([":" + allowlist_test, ":" + overlap_test])},
        )
    else:
        native.test_suite(
            name = name,
