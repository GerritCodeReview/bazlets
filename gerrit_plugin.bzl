...
def gerrit_plugin_ext_deps(
    ...
    Args:
      ...
      plugin: Name of the plugin under test. Used to derive the plugin library
        dependency and as a prefix for generated target names.
      ext_deps: List of Maven coordinates for external test utility dependencies.
        Requires `plugin` to be set.
      ext_repo: Name of the external repository generated by rules_jvm_external.
        Defaults to `<plugin>_plugin_deps`.
    ...
    if not plugin:
        fail("gerrit_plugin_ext_deps: `plugin` must be set when `ext_deps` is provided")

    if name == None:
        name = plugin + "__plugin_ext_deps"
...
def gerrit_plugin_test_util(
    ...
    if plugin:
        if name == None:
            name = plugin + "__plugin_test_util"
        if ext_repo == None:
            ext_repo = plugin + "_plugin_deps"
    elif name == None:
        fail("gerrit_plugin_test_util: `name` must be set when `plugin` is empty")
    ...
    if ext_deps:
        if not plugin:
            fail("gerrit_plugin_test_util: `plugin` must be set when `ext_deps` is provided")

        gerrit_plugin_ext_deps(
            name = plugin + "__plugin_test_util_deps",
            plugin = plugin,
            ext_deps = ext_deps,
            ext_repo = ext_repo)
        deps = deps + [":" + plugin + "__plugin_test_util_deps"]
