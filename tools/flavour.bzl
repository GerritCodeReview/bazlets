...
rows on one classpath:
    ...
    ee10      jakarta.servlet 6.0     Jetty EE10           7
...
`//flags:ee10` is the config_setting that matches when the flag equals `ee10`,
...
# IMPORTANT: today's `ee10` is really EE11

The current jakarta tier is wired to `jakarta.servlet` **6.1.0**. Per the table
above, Servlet 6.1 is **EE11** (Jetty EE11) -- **not** EE10, which would be
Servlet 6.0. So the `ee10` spelling used throughout these helpers is a known
**misnomer**, kept only for continuity; the honest name is `ee11`, and a rename
is expected. Genuinely targeting EE10 would mean *downgrading* to Servlet 6.0,
which is not the goal. EE12 (Jetty 13 / Servlet 7.0) is anticipated to be added
*alongside* EE11, not to replace it. Until the rename lands, read `ee10` here as
"the active jakarta flavour".
...
via the `flavour` attribute):
  ...
  * flavoured_jar      -- builds `actual` under that transition and republishes
    its jar. gerrit_plugin(flavour = "ee10") wraps its output in this, so
    `bazelisk build //plugins/foo:foo-ee10` self-selects the jakarta config and
  ...
  * flavoured_war      -- same wrapper for a pkg_war, so release-ee10.war /
    headless-ee10.war build under the transition next to the EE8 WARs.
...
on the bazlets `//flags:ee10` config_setting. (Gerrit's `//tools:ee10` is an
...
def flavour_only(*flavours):
    ...
    Args:
      *flavours: one or more flavour names (e.g. "ee10"), each mapped to its
...
def flavoured_twin_alias(name, flavours, default_flavour = "ee8", **kwargs):
    ...
    `httpd` -> `:httpd-ee8` / `:httpd-ee10`): list only the jakarta `flavours`
...
def flavoured_library(
    ...
    flavour is NOT a pure transform -- e.g. `pgm/http/jetty`, whose ee10 twin
    ...
    Args:
      ...
      flavours: non-default flavour names to generate twins for. Defaults to
        ...
        sources. (Today `to_jakarta` maps javax->jakarta and Jetty ee8->ee10;
        jakarta.servlet is version-stable so this serves ee10/ee11/ee12, but a
...
def flavoured_tests(
    ...
    Hand-written test fakes that diverge -- e.g. the util/http testutil ee10
...
flavoured_jar = rule(
    ...
    attrs = {
        ...
        "flavour": attr.string(
            ...
            doc = "Flavour to force for `actual`, e.g. \"ee10\".",
        ),
...
flavoured_war = rule(
    ...
    attrs = {
        ...
        "flavour": attr.string(
            mandatory = True,
            values = KNOWN_FLAVOURS,
            doc = "Flavour to force for `war`, e.g. \"ee10\".",
        ),
        "_allowlist_function_transition": attr.label(
            default = "@bazel_tools//tools/allowlists/function_transition_allowlist",
        ),
