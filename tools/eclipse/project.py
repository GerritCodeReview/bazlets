...
class EclipseProject():
  ...
  def main(self, args=sys.argv[1:]):
    ...
    self.bazel_exe = self.find_bazel(args.bazel_exe)
    self.batch = args.batch
    self.exclude = args.exclude

    self.ROOT = self.find_root(args.root)
    project_name = args.name if args.name else os.path.basename(self.ROOT)
  ...
  def _parse_args(self, args):
    self._get_argument_parser().parse_args(args)
  ...
  def find_root(self, root):
    ...
    while not os.path.exists(os.path.join(ROOT, 'WORKSPACE')):
      ...
      if ROOT == '/':
        raise Exception(
          "Could not find any WORKSPACE file in parent directories")
    return ROOT
  ...
  def _query_classpath(self):
    ...
    try:
      subprocess.check_call(self._build_bazel_cmd('build', t))
    except subprocess.CalledProcessError:
      exit(1)
